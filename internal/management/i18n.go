...
var catalogs = map[Lang]map[string]string{
	LangZhHant: {
		...
		"col_weekly":         "週限",
		"col_status":         "狀態",
		"accounts_hint":      "5h／週限來自 CPA 既有資料；沒有就顯示 —，不會自己推估。額度欄只供參考，不決定是否 ping。",
		"actions":            "動作",
	...
	LangEn: {
		...
		"col_weekly":         "Weekly",
		"col_status":         "Status",
		"accounts_hint":      "5h / weekly figures come from existing CPA data; missing values show as — and are never estimated. Quota is informational and does not decide whether to ping.",
		"actions":            "Actions",
	...
	LangJa: {
		...
		"col_weekly":         "週次",
		"col_status":         "状態",
		"accounts_hint":      "5h／週次は CPA 既存データ由来です。無い場合は — と表示し、推測しません。クォータ欄は参考情報で、ping 可否は決めません。",
		"actions":            "操作",
